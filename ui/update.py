...
def run_up():
    ...
    # 사용자 입력 받기
    st.subheader("기본 정보")
    name = st.text_input("이름", key="name_up", help="상담을 예약한 분의 이름을 입력하세요.")
    phone = st.text_input("연락처", key="phone_up", help="상담을 예약한 분의 연락처를 입력하세요.")

    st.subheader("상담 예약 정보")
    sel_date = st.date_input("상담 날짜", value=date.today() + timedelta(days=1), key="date_up", help="상담을 예약한 날짜를 선택하세요.")
    times = [f"{hour:02d}:00" for hour in [10, 11, 13, 14, 15, 16, 17]]
    time = st.selectbox("상담 시간", times, key="time_up", help="상담을 예약한 시간을 선택하세요.")
    ...
    if name and phone and sel_date and time:
        ...
        if valid:
            ...
            if not matching_rows:
                ...
                prev_issue = matching_rows[0]['요청']
                prev_category = matching_rows[0]['카테고리']
                
                st.write("새로운 상담 시간과 내용을 입력해주세요.")
                valid = False
                ...
                new_date = sel_date.strftime("%Y-%m-%d") + " " + time
                new_date = datetime.strptime(new_date, "%Y-%m-%d %H:%M")
                issue = st.text_area("새로운 상담 요청 내용 (선택)")

                if st.button("예약 변경"):
                    if sel_date and time:
                        if new_date > datetime.now():
                            valid = True
                        if valid:
                            if issue:
                                ...
                                # 해당 날짜 해당 시간에 해당 카테고리 변호사의 상담이 이미 예약되어 있는지 확인
                                records = sheet.get_all_records()  # 시트 데이터를 가져옴
                                lawyer_list = lawyers.get(prev_category)
                                for lawyer in lawyer_list:
                                    matching_rows = [
                                        row for row in records
                                        if row['날짜'] == str(sel_date) and row['시간'] == time and row['변호사명'] == lawyer
                            ...
                            if valid:
                                if issue:
                                    ...
                                    sheet.append_row([name, phone, str(sel_date), str(time), prev_issue, prev_category, lawyer])
                                    # 결과 출력
                                    st.success(
                                        f"✅ **{prev_category}** 관련 사안에 대해 문의 주셨습니다.\n\n"
                                        f"📅 **{sel_date} {time}**로 예약 변경이 완료되었습니다.\n\n"
                                        f"👨‍⚖️ 담당 변호사 : **{lawyer}**"
                                    )
                                # 기존 데이터 삭제
