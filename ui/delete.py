...
def run_del():
    ...
    # 사용자 입력 받기
    name = st.text_input("이름", key="name_del")
    phone = st.text_input("연락처", key="phone_del")
    sel_date = st.date_input("상담 날짜", value=date.today() + timedelta(days=1), key="date_del")
    times = [f"{hour:02d}:00" for hour in [10, 11, 13, 14, 15, 16, 17]]
    time = st.selectbox("상담 시간", times, key="time_del")
